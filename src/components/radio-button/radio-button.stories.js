...
const formStyles = `<style>
  *,*::before,*::after{box-sizing:border-box}
  ...
  .check-input{width:20px;height:20px;min-width:20px;border:2px solid var(--color-charcoal-40);border-radius:50%;background:var(--color-white);appearance:none;-webkit-appearance:none;cursor:pointer;transition:background var(--duration-fast) var(--ease-out),border-color var(--duration-fast) var(--ease-out);position:relative;flex-shrink:0;margin-top:2px;outline:none}
  ...
  .check-input:checked::after{content:'';position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:8px;height:8px;background:white;border-radius:50%}
...
<div style="padding:var(--sp-48);font-family:var(--font-sans);background:var(--color-off-white);min-height:100vh">
  ...
  <p style="color:var(--color-text-secondary);margin-block-end:var(--sp-48)">Use for mutually exclusive single-select options. All radios in a group share the same <code>name</code> attribute. Checked state fills with <code>--color-action-primary</code> and a centred white dot via <code>::after</code>.</p>
    ...
    <div class="form-group">
      ...
      <div class="check-list">
        <label class="check-group">
          <input class="check-input" type="radio" name="sb-year" checked>
          <span class="check-label">September 2025</span>
        </label>
        <label class="check-group">
          <input class="check-input" type="radio" name="sb-year">
          <span class="check-label">January 2026</span>
        </label>
        <label class="check-group">
          <input class="check-input" type="radio" name="sb-year">
          <span class="check-label">September 2026</span>
        </label>
      </div>
    </div>
    <div class="form-group">
      ...
      <div class="check-list">
        <label class="check-group">
          <input class="check-input" type="radio" name="sb-mode" checked disabled>
          <span class="check-label check-label--disabled">Full-time (pre-selected)</span>
        </label>
        <label class="check-group">
          <input class="check-input" type="radio" name="sb-mode" disabled>
          <span class="check-label check-label--disabled">Part-time (not available)</span>
        </label>
      </div>
    </div>
  </div>
  ...
  ${statesTable([
    ...
    ['Selected',          'var(--color-action-primary)','--color-action-primary', 'bg: forest-green · border: forest-green · 8px white dot centred via ::after'],
    ['Selected Hover',    'var(--color-action-primary)','--color-action-primary', 'bg: forest-green · opacity: var(--opacity-hover, 0.8)'],
...
<div style="padding:var(--sp-48);font-family:var(--font-sans);background:var(--color-off-white);min-height:100vh">
  ...
  <p style="color:var(--color-text-secondary);margin-block-end:var(--sp-48)">Error state when a required radio group has no selection. Apply <code>--error</code> border to all radios in the group and render the error message beneath the group.</p>
  ...
  <div style="background:var(--color-white);border-radius:var(--radius-m);border:1px solid var(--color-border);padding:var(--sp-32);margin-block-end:var(--sp-48);max-width:400px">
    <div class="form-group">
      ...
      <div class="check-list" style="margin-block-end:var(--sp-8)">
        <label class="check-group">
          <input class="check-input check-input--error" type="radio" name="sb-year-err" aria-invalid="true" aria-describedby="sb-radio-err">
          <span class="check-label">September 2025</span>
        </label>
        <label class="check-group">
          <input class="check-input check-input--error" type="radio" name="sb-year-err" aria-invalid="true">
          <span class="check-label">January 2026</span>
        </label>
        <label class="check-group">
          <input class="check-input check-input--error" type="radio" name="sb-year-err" aria-invalid="true">
